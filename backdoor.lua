...
function notify(entity, bypassed, other)
	...
	if bypassed then
		tem.TextLabel.Text = entity .. " 已被绕过!"
	end
	if other then
...
workspace.ChildAdded:Connect(function(child)
	if child.Name == "BackdoorLookman" then
		...
		notify("👁️ Backdoor窥探者")
		selection(child:WaitForChild("Core"), "👁️ Backdoor窥探者")
	elseif child.Name == "后门Rush" then
...
for _, v in pairs(workspace.CurrentRooms:GetDescendants()) do
	...
	elseif v.Name == "后门_柜子" then
		selection(v, "🗄️ 柜子")
	elseif v.Name == "计时器Lever" then
		selection(v, "⏳ 计时器 Lever")
		spawn(function()
			while task.wait() do
				if checkDistance(v:FindFirstChildWhichIsA("BasePart")) then
...
workspace.CurrentRooms.ChildAdded:Connect(function(child)
	spawn(function()
		task.wait(5)
		if child.Assets:FindFirstChild("计时器Lever") then
			selection(child.Assets.TimerLever, "⏳ 计时器 Lever")
			spawn(function()
				while task.wait() do
	...
	for _, v in pairs(child:GetDescendants()) do
		...
		elseif v.Name == "后门_柜子" then
			selection(v, "🗄️ 柜子")
		elseif v.Name == "ActivateEventPrompt" and v:IsA("ProximityPrompt") and v.Parent.Parent.Name == "抽屉容器" then
			spawn(function()
				while task.wait() do
					if checkDistance(v:FindFirstChildWhichIsA("BasePart")) then
						fireproximityprompt(v)
...
notify("", nil, "🐦 脚本作者: kiwib 脚本汉化者：云")
notify("👁️ Backdoor Lookman", true)
notify("", nil, "⚡ Haste因为现在是服务器端的，所以无法绕过，已删除视觉效果.")
